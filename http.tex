...
The current \API uses two methods: \GET and \POST. As per \HTTP standards, \HTTP \GET is a \emph{safe} method which means it is intended only for information retrieval and should not change the state of the server. The \HTTP \POST methods on the other hand results in a remote function call or script execution which does change the server state.
...
\texttt{GET}    & object  & retrieve      & format parameters     & \texttt{GET /ocpu/library/datasets/R/mtcars/json}            \\
                & graphic & render        & rendering parameters  & \texttt{GET /ocpu/tmp/x09362873/graphics/1/png}            \\
                & file    & download      & -                     & \texttt{GET /ocpu/library/MASS/NEWS}                         \\
                & dir     & list contents & -                     & \texttt{GET /ocpu/library/MASS/scripts/}                     \\ \bottomrule
