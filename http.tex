...
This section introduces the \OpenCPU system in more detail. At this point the general logic and concerns discussed earlier become more concrete as we illustrate how the pieces come together in the context of \R and \HTTP. In this paper we mostly focus on the interface of the system and only touch on implementation details to the extend that they are important to appreciate the logic. Besides explaining how clients are supposed to interact with the system, the interface embodies the separation of concerns that is the central to this work. The concepts and operations defined by the \API suggest a minimal structure for embedded scientific computing. 

For the purpose of this paper, just as important as what is included in the \API, is what was left out of the \API. For example the \RPC mechanism implies that concerns of managing processes, hardware and security are at the discretion of the server. Because clients can specify the data interchange format for retrieving an arbitrary resource, the server needs to implement mappings between the various resource types and data interchange formats. 


%implicity by left out.
...
The following are common \HTTP status codes returned by \OpenCPU that the client should be able to interpret. The meaning of these status codes is conform \HTTP standards. The web server might use additional status codes that are not specific to \OpenCPU.
...
A client can retrieve objects on the server in various \emph{formats} by postfixing the object \URL with the format identifier in the \GET request. Which formats are supported and how an object is exported into a particular format is at the discretion of the server implementation. Also not every format supports every object type. For example, \texttt{csv} can only be used to retrieve tabular data structures, and \texttt{png} is only appropriate for graphics. Table \ref{table:formats} lists the formats \OpenCPU supports, the respective \texttt{Content-type}, and the \R function that our implementation uses to convert an object into a particular format. Arguments of the \GET requests are mapped to the respective export function. For example the \texttt{png} format has parameters such as \texttt{width} and \texttt{height}, whereas the \texttt{tab delimited} format has parameters \texttt{sep}, \texttt{eol}, \texttt{dec} which specify the delimiting, end-of-line and decimal character respectively.
...
The root of the API is dynamic. It defaults to \texttt{/ocpu/} however system administrators can change this. Clients should make the \OpenCPU server address and root path configurable. In the examples below we assume the default \texttt{/ocpu/} is used.
...
\begin{tabular}{@{}lll@{}}
\toprule
Path & Desription                      & Examples                \\ \midrule
...
\toprule
\texttt{Content-type}                      & Primitives & Data structures  & Raw code & File & Temp Key \\ \midrule
