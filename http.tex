...
This section introduces the \OpenCPU system in more detail. At this point the general logic and concerns discussed earlier become more concrete as we illustrate how the pieces come together in the context of \R and \HTTP. In this paper we mostly focus on the interface of the system and only touch on implementation details to the extend that they are relevant to the design of the system. Besides explaining how clients are supposed to interact with the system, the interface embodies the separation of concerns that is the central to this work. Therefore just as relevant as what is included in the \API, is what was left out of the interface. For example the \RPC mechanism implies that concerns of managing processes, hardware and security are at the discretion of the server. Moreover, because clients can specify the data interchange format for retrieving an arbitrary resource, the server needs to implement mappings between the various resource types and data interchange formats. And because computational exceptions are returned using \HTTP status codes, error handling is the responsibility of the client. These are some examples of how the \API implicitly gives rise to the boundaries for separation concerns. Thereby the concepts and operations defined by the \API suggest a minimal structure for embedded scientific computing. 
